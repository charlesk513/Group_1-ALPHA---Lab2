...
public class Bench {
    ...
    public static void main(final String[] args) {
        // executionTimeReport("Quick.java: quicksort", new Quick(false, false,
        // 0)::sort);
        executionTimeReport("Quick.java: quicksort with all improvements", new Quick(true, true, 40)::sort);
    ...
    private static void executionTimeReport(String name, Consumer<int[]> algorithm) {
        ...
        Function<Integer, Function<Object, String>> pad = n -> x -> {
            if (n <= 0)
                return x.toString();
            else
                return String.format("%" + n + "s", x);
        };
        Function<Integer, Function<Object, String>> centre = n -> x -> {
        ...
        BiFunction<String, Stream<String>, String> makeLine = (delim, s) -> s.collect(Collectors.joining(delim));
        ...
        BiFunction<Object, Stream<Object>, String> makeRow = (size, stream) -> makeLine.apply(" || ",
                Stream.concat(Arrays.asList(size).stream().map(pad.apply(sizeLength)),
                        stream.map(centre.apply(timeLength))));

        Function<Integer, Function<Double, String>> formatTime = n -> t -> String.format("%9.1f | %8.4f", t * 1000000,
                t * 1000000 / n);
        ...
        runWithLargeStack(32 * 1024 * 1024, () -> {
            try {
                ...
                System.out.println(
                        makeRow.apply("", Arrays.stream(RANDOMNESS).mapToObj(r -> getRandomnessDescription(r))));
                System.out.println(
                        makeRow.apply("Size", Collections.nCopies(RANDOMNESS.length, (Object) header).stream()));
                System.out.println(sep);
                for (int size : SAMPLE_SIZES)
                    System.out.println(makeRow.apply(size, Arrays.stream(RANDOMNESS)
                            .mapToObj(r -> formatTime.apply(size).apply(execute(algorithm, generateSample(size, r))))));
